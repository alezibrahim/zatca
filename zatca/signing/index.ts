...
export const getPureInvoiceString = (invoice_xml: XMLDocument): string => {
  ...
  const invoice_xml_dom = new xmldom.DOMParser().parseFromString(invoice_copy.toString({ no_header: false }));
...
export const getCertificateHash = (certificate_string: string): string => {
  const certificate_hash = Buffer.from(createHash("sha256").update(certificate_string).digest("hex")).toString(
    "base64"
  );
...
export const createInvoiceDigitalSignature = (invoice_hash: string, private_key_string: string): string => {
  ...
  const wrapped_private_key_string: string =
  ...
  //var signature = new Buffer(sign.sign(wrapped_private_key_string).toString("base64"), "base64").toString();
  return signature;
};
...
export const getCertificateInfo = (
...
): { hash: string; issuer: string; serial_number: string; public_key: Buffer; signature: Buffer } => {
  ...
  //console.log(wrapped_certificate_string);
  ...
  //console.log(x509.issuer.split("\n").reverse().join(", "));
  //console.log(BigInt(`0x${x509.serialNumber}`).toString(10));
  return {
    hash: hash,
...
export const generateSignedXMLString = ({
  ...
  certificate_string,
  private_key_string,
}: generateSignatureXMLParams): { signed_invoice_string: string; invoice_hash: string; qr: string } => {
  const invoice_copy: XMLDocument = new XMLDocument(invoice_xml.toString({ no_header: false }));
  //console.log(invoice_xml.toString({ no_header: false }));
  // 1: Invoice Hash
  ...
  //display xml ahmed
  //console.log(signed_invoice.toString({ no_header: false }));
